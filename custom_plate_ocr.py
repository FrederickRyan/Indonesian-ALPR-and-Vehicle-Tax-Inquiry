        ...
        try:
            ...
            if use_4bit and torch.cuda.is_available():
                bnb_config = BitsAndBytesConfig(
                    ...
                    llm_int8_enable_fp32_cpu_offload=True  # Allow CPU offloading for larger models
                )
                
                # For GPUs with <= 4GB VRAM, use max_memory to control allocation
                if gpu_mem <= 5:
                    max_memory = {0: f"{int(gpu_mem * 0.85)}GB", "cpu": "12GB"}
                    print(f"Using memory mapping: GPU={int(gpu_mem * 0.85)}GB, CPU=12GB")
                else:
                    max_memory = None
            else:
                max_memory = None
